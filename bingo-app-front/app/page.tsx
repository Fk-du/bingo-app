...
export default function Home() {
  ...
  // Always authenticate with the fresh initData on every launch. The persisted
  // session may belong to a different Telegram account (WebView storage is
  // shared across accounts on the same device), so it must never be trusted.
  useEffect(() => {
    if (!tgReady || didAutoLogin.current) return;
  ...
  return (
    <div className="flex flex-col items-center justify-center min-h-screen gap-6 p-8">
      ...
      {error && (
        <p className="text-rose-500 mt-2">{error.message || 'Authentication failed. Please try again.'}</p>
      )}
    </div>
  );
}
