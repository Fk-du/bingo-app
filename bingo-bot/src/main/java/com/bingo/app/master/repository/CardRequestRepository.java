...
public interface CardRequestRepository extends JpaRepository<CardRequest, Long> {
    ...
    Optional<CardRequest> findFirstByAdminUserIdAndStatusOrderByCreatedAtDesc(Long adminUserId, FundStatus status);

    List<CardRequest> findByAdminUserIdOrderByCreatedAtDesc(Long adminUserId);
    ...
    @Query("UPDATE CardRequest r SET r.status = :status, r.approvedBy = :approvedBy, " +
